    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('support_ticket.admin_store') }}" method="post" id="ticket-reply-form" enctype="multipart/form-data">
                ...
                <input type="hidden" name="status" value="{{ $ticket->tic_status }}" required>
                <div class="form-group">
                    <textarea class="form-control" data-buttons='[["font", ["bold", "underline", "italic"]],["para", ["ul", "ol"]],["view", ["undo","redo"]]]' rows=5 name="reply" required></textarea>
                ...
                <div class="form-group mb-0 text-right">
                    <!-- <button type="submit" class="btn btn-sm btn-dark" onclick="submit_reply('pending')" name="pendingBtn">
                        {{ translate('Submit as') }} 
                        <strong>
                    ...
                    <button type="submit" class="btn btn-icon btn-sm btn-dark" id="dropdownMenuButton" name="is_not_pendingBtn" data-toggle="dropdown" aria-haspopup="true"><i class="las la-angle-down"></i></button>
                    ...
                    <button type="submit" class="btn btn-sm btn-primary" name="openBtn" value="open">Open</button>
                    <button type="submit" class="btn btn-sm btn-secondary" name="solveBtn" value="solved">Solved</button>
                    <button type="submit" class="btn btn-sm btn-danger" name="viewBtn" value="viewed">Viewed</button>
                </div>
            </form>
            <div class="pad-top">
                ...
                <hr>
                <a href="/support_ticket/show_files/{{$ticket_info->files}}" >{{$ticket_info->files}}</a>
            </div>
        </div>
    </div>
</div>
