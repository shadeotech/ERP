        ...
        <div class="col-lg-8 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h1 class="h6 mb-0">{{ translate('General Settings') }}</h1>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            <label class="col-sm-3 col-from-label">{{ translate('Invoice Logo') }}</label>
                            <div class="col-sm-9">
                                <div class="input-group" data-toggle="aizuploader" data-type="image">
                                    <div class="input-group-prepend">
                                        <div class="input-group-text bg-soft-secondary">{{ translate('Browse') }}</div>
                                    ...
                                    <input type="hidden" name="invoice_logo" value="{{ get_setting('invoice_logo') }}" class="selected-files">
                                </div>
                                <div class="file-preview box sm"></div>
                                <small>{{ translate('Will be used in invoices as logo') }}</small>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                ...
                                <select name="timezone" class="form-control aiz-selectpicker" data-live-search="true">
                                    @foreach (timezones() as $key => $value)
                                        <option value="{{ $value }}" @if (app_timezone() == $value) selected @endif>{{ $key }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
