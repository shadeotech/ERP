                ...
                <tbody>
                    @foreach($cassettes as $item)
                        <tr>
                            ...
                            <td style="vertical-align:middle;">
                                @if($item->archived == 0)
                                <a href="{{route('cassette.destroy', $item->id)}}" class="btn btn-soft-danger btn-icon btn-circle btn-sm "  title="Delete" onclick='return confirm("Are you sure you want to delete this item?")'><i class="las la-trash"></i>
                                </a>
                                @else
