    ...
    <form action="{{ route('seller.profile.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="h6 mb-0">Social Media</h5>
            </div>
            <div class="card-body">
                <div class="form-group row">
                    <label class="col-md-2 col-form-label">Website</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="Website Address" name="website" @isset($profile_data->website) value="{{ $profile_data->website }}" @endisset>
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-md-2 col-form-label">Facebook</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="Facebook" name="facebook" @isset($profile_data->facebook) value="{{ $profile_data->facebook }}" @endisset>
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-md-2 col-form-label">Twitter</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="Twitter" name="twitter" @isset($profile_data->twitter) value="{{ $profile_data->twitter }}" @endisset>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <div class="input-group" data-toggle="aizuploader" data-type="image">
                            ...
                            <input type="hidden" name="logo" value="{{ Auth::user()->logo }}" class="selected-files">
                        </div>
                    ...
                    @foreach (Auth::user()->addresses as $key => $address)
                        <div class="col-lg-6">
                            <div class="position-relative mb-3 rounded border p-3 pr-5">
                                ...
                                <div>
                                    <span class="w-50 fw-600">{{ translate('Sate') }}:</span>
        ...
        <!--div class="card">
                  ...
                  <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-9">
                            <label class="aiz-switch aiz-switch-success mb-3">
                                <input value="1" name="cash_on_delivery_status" type="checkbox" @if (Auth::user()->seller && Auth::user()->seller->cash_on_delivery_status == 1) checked @endif>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-9">
                            <label class="aiz-switch aiz-switch-success mb-3">
                                <input value="1" name="bank_payment_status" type="checkbox" @if (Auth::user()->seller && Auth::user()->seller->bank_payment_status == 1) checked @endif>
                                <span class="slider round"></span>
                            </label>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-md-9">
                            <input type="text" class="form-control mb-3" placeholder="{{ translate('Bank Name') }}" value="{{ Auth::user()->seller && Auth::user()->seller->bank_name }}" name="bank_name">
                        </div>
                    </div>
                    <div class="row">
                        <label class="col-md-3 col-form-label">{{ translate('Bank Account Name') }}</label>
                        <div class="col-md-9">
                            <input type="text" class="form-control mb-3" placeholder="{{ translate('Bank Account Name') }}" value="{{ Auth::user()->seller && Auth::user()->seller->bank_acc_name }}" name="bank_acc_name">
                        </div>
                    </div>
                    <div class="row">
                        <label class="col-md-3 col-form-label">{{ translate('Bank Account Number') }}</label>
                        <div class="col-md-9">
                            <input type="text" class="form-control mb-3" placeholder="{{ translate('Bank Account Number') }}" value="{{ Auth::user()->seller && Auth::user()->seller->bank_acc_no }}" name="bank_acc_no">
                        </div>
                    </div>
                    <div class="row">
                        <label class="col-md-3 col-form-label">{{ translate('Bank Routing Number') }}</label>
                        <div class="col-md-9">
                            <input type="number" lang="en" class="form-control mb-3" placeholder="{{ translate('Bank Routing Number') }}" value="{{ Auth::user()->seller && Auth::user()->seller->bank_routing_no }}" name="bank_routing_no">
                        </div>
                    </div>
                  </div>
              </div-->
                    ...
                    <div class="modal-body">
                        <div class="p-3">
                                ...
                                <div class="col-md-10">
                                    <div class="mb-3">
                                        <select class="form-control aiz-selectpicker" data-live-search="true" data-placeholder="{{ translate('Select your country') }}" name="country" required>
                                            ...
                                            @foreach (\App\Models\Destination::select('country', 'country_code')->distinct()->get() as $key => $country)
                                                <option value="{{ $country->country }}"  data-country="{{ $country->country_code }}" >{{ $country->country }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-10">
                                    <select class="form-control aiz-selectpicker mb-3" data-live-search="true" name="state" required>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-10">
                                    <input type="text" onkeypress="return /[a-z ]/i.test(event.key)" placeholder="City" id="city" name="city" class="form-control mb-3" required>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        ...
        function edit_address(address) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    editAddressData = response.data.address_data;
                    console.log(response);
                    $('#edit_modal_body').html(response.html);
                    $('#edit-address-modal').modal('show');
                    AIZ.plugins.bootstrapSelect('refresh');
                    var country = $("#edit_country option:selected").data("country");
                    get_city(country);
                }
            });
        }
        ...
        function get_city(country) {
            ...
            $.ajax({
                ...
                success: function(response) { 
                    if (response != '') {
                        $('[name="state"]').html(response);
                    } else {
                        $('[name="state"]').html("");
                    }
                    if(editAddressData && editAddressData.state) {
                        $('[name="state"]').val(editAddressData.state);
                    }
                    AIZ.plugins.bootstrapSelect('refresh');
                }
            });
