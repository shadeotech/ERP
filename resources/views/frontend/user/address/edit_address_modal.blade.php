<form class="form-default" role="form" action="{{ route('addresses.update', $address_data->id) }}" method="POST">
    ...
    <div class="p-3">
                ...
                <div class="mb-3">
                    <select class="form-control aiz-selectpicker" data-live-search="true" data-placeholder="{{ translate('Select your country') }}" name="country" id="edit_country" required>
                        @foreach (\App\Models\Destination::select('country', 'country_code')->distinct()->get() as $key => $country)
                            <option value="{{ $country->country }}" @if ($address_data->country == $country->country) selected @endif data-country="{{ $country->country_code }}">{{ $country->country }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-md-10">
                <input type="text" class="form-control mb-3" placeholder="{{ translate('City') }}" value="{{ $address_data->city }}" name="city" value="" required>
            </div>
        </div>
