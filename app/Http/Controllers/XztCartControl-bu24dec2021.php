...
class XztCartControl extends Controller
{
    ...
    public function store_cart(Request $request) {
        ...
        try {
            DB::transaction(function () use ($request, &$success, &$order, &$ship_addr) {
                $ship_addr = new XztShippingAddr();
        ...
        $this->send_inv_seller($order->id, $ship_addr->id);
        if($success){
            echo "success";
        }else {
            echo "failed";
    ...
    public function send_inv_seller($order_id, $ship_id){
        $cart_items = Xztcart::where('order_id', $order_id)->get();
        $ship = XztShippingAddr::where('id', $ship_id)->first();
        $order = Order::where('id', $order_id)->first();

        Mail::send('emails/inv_template', ['cart_items' => $cart_items, 'ship' => $ship, 'order' => $order], 
            function($messages) use ($ship) {
            $messages->from(env('MAIL_USERNAME'), 'Shadeotech');
            $messages->to($ship->email, 'Receiver')->subject("Shadeotech Order");
        });
    }
