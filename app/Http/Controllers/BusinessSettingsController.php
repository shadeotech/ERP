...
class BusinessSettingsController extends Controller
{
    ...
    {
        $templates = [
            "inv_template.txt" => "Invoice Email Template Content",
            "quote_template.txt" => "Quote Email Template Content",
            "verification.txt" => "Verification Email Template Content",
        ];
        ...
        foreach ($email_files as $file) {
            ...
            $template->{"name"} = $templates[$f[array_key_last($f)]];
            $template->{"file_name"} = $f[array_key_last($f)];

            array_push($files, $template);
        }

        return view('backend.setup_configurations.email_content', compact('files'));
    }

    public function email_content_save(Request $request)
    {
        $templates = [
            "inv_template.txt",
            "quote_template.txt",
        ...
        foreach ($templates as $file) {
            $name_value = str_replace(".", "_", $file);
            $content = htmlspecialchars_decode(trim($request->{$name_value}));
            Storage::put("emails_content/" . $file, $content);
        }

        flash(translate("Emails content updated successfully"))->success();
        return back();
    }
