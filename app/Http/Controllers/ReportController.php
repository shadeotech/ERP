...
class ReportController extends Controller
{
    ...
    {
        $sort_by =null;
        $sold = Xztcart::with('product')->select('product_id', DB::raw('sum(quantity) as sold'))->groupBy('product_id');
        $categories = Category::where('parent_id', 1)->orwhere('parent_id', 0)->with('childrenCategories')->get();
 
        if ($request->has('category_id')) {
            $searchcat = $request->category_id;
            $get_ids = Category::where('parent_id', $searchcat)->pluck('id');
            $products = Product::whereIn('category_id', $get_ids)->pluck('id');
            $sold = $sold->whereIn('product_id', $products);
        }

        $sold = $sold->paginate(15);    
        return view('backend.reports.in_house_sale_report', compact('sort_by', 'sold', 'categories'));
    }
