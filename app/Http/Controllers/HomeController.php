...
class HomeController extends Controller
{
     ...
     * @return double
     */
    public function getOrdersSumForDate($orders, $date)
    {
        $sum = 0;
        foreach ($orders as $order) {
            if (Carbon::parse($order->created_at)->toDateString() == Carbon::parse($date)->toDateString()) {
    ...
    public function admin_dashboard()
    {
        ...
        $tickets = Ticket::whereHas("user")->count();

        $last12MonthDates = [];
        $last12MonthsPurchases = [];
        for ($i = today()->subYear()->addMonth(); $i <= today(); $i->addMonth()) {
            $date = $i->copy();
            $value = Order::whereDate("created_at", ">=", $date->startOfMonth())->whereDate("created_at", "<=", $date->endOfMonth())->sum("grand_total");
            array_push($last12MonthDates, $i->format("Y-m"));
            array_push($last12MonthsPurchases, round($value, 2));
        }
        $last180DaysDates = [];
        $last180DaysPurchases = [];
        $allOrderData = Order::select("created_at", "grand_total")->get();
        for ($i = today()->subDays(179); $i <= today(); $i->addDay()) {
            $date = $i->copy()->format("Y-m-d");
            $value = $this->getOrdersSumForDate($allOrderData, $date);
            array_push($last180DaysDates, $i->format("Y-m-d"));
            array_push($last180DaysPurchases, $value);
        }
        $last120DaysDates = array_slice($last180DaysDates, 180 - 120 - 1, 120);
        $last120DaysPurchases = array_slice($last180DaysPurchases, 180 - 120 - 1, 120);

        $last90DaysDates = array_slice($last180DaysDates, 180 - 90 - 1, 90);
        $last90DaysPurchases = array_slice($last180DaysPurchases, 180 - 90 - 1, 90);

        $last60DaysDates = array_slice($last180DaysDates, 180 - 60 - 1, 60);
        $last60DaysPurchases = array_slice($last180DaysPurchases, 180 - 60 - 1, 60);

        $last30DaysDates = array_slice($last180DaysDates, 180 - 30 - 1, 30);
        $last30DaysPurchases = array_slice($last180DaysPurchases, 180 - 30 - 1, 30);

        $last5YearsDates = [];
        $last5YearsData = [];
        for ($i = today()->subYears(5); $i <= today()->endOfYear(); $i->addYear()) {
            $date = $i->copy();
            $value = Order::whereDate("created_at", ">=", $date->startOfYear())->whereDate("created_at", "<=", $date->endOfYear())->sum("grand_total");
            if ($value > 0 || sizeof($last5YearsData) > 0) {
                array_push($last5YearsDates, $i->format("Y"));
                array_push($last5YearsData, round($value, 2));
            }
        }

        //Products and Category
        $allPurchasedProducts = Xztcart::with('product')->get();
        $allCategories = Category::all();

        $categoryPurchases = collect();

        foreach ($allCategories as $cat) {
            $sum = 0;
            foreach ($allPurchasedProducts as $item) {
                if ($item->product->category_id == $cat->id) {
            ...
            if ($sum > 0) {
                $categoryPurchases->add([
                    "name" => $cat->name,
                    "purchase" => round($sum, 2),
                ]);
            }
        }
        $categoryPurchases = $categoryPurchases->sortByDesc("purchase");
        $categoryPurchases = $categoryPurchases->slice(0, 5)->all();

        $categoryNames = [];
        $categoryPurchaes = [];
        if ($categoryPurchases && sizeof($categoryPurchases) > 0) {

            foreach ($categoryPurchases as $c) {
                array_push($categoryNames, $c["name"]);
                array_push($categoryPurchaes, $c["purchase"]);
            }
        }

        return view('backend.dashboard', compact('dealers', 'tickets', 'last180DaysDates', 'last180DaysPurchases', 'last120DaysDates', 'last120DaysPurchases', 'last90DaysDates', 'last90DaysPurchases', 'last60DaysDates', 'last60DaysPurchases', 'last30DaysDates', 'last30DaysPurchases', 'categoryNames', 'categoryPurchaes', 'last5YearsDates', 'last5YearsData', 'orders', 'categories', 'products', 'last12MonthDates', 'last12MonthsPurchases'));
    }
    ...
    public function dashboard()
    {
        ...
        $t_orders_pending = Order::where('status', 'Pending')->where('user_id', Auth::user()->id)->count();
        $t_orders_cancelled = Order::where('status', 'Cancelled')->where('user_id', Auth::user()->id)->count();
        $t_orders_success = Order::where('status', 'Ready')->where('user_id', Auth::user()->id)->count();
        $t_products = Product::where('state', 'Active')->count();
        // dd($t_orders_pending);
        $last12MonthDates = [];
        $last12MonthsPurchases = [];
        for ($i = today()->subYear()->addMonth(); $i <= today(); $i->addMonth()) {
            $date = $i->copy();
            $value = Order::where('user_id', Auth::user()->id)->whereDate("created_at", ">=", $date->startOfMonth())->whereDate("created_at", "<=", $date->endOfMonth())->sum("grand_total");
            array_push($last12MonthDates, $i->format("Y-m"));
            array_push($last12MonthsPurchases, round($value, 2));
        }
        ...
        for ($i = today()->subYears(5); $i <= today()->endOfYear(); $i->addYear()) {
            $date = $i->copy();
            $value = Order::where('user_id', Auth::user()->id)->whereDate("created_at", ">=", $date->startOfYear())->whereDate("created_at", "<=", $date->endOfYear())->sum("grand_total");
            if ($value > 0 || sizeof($last5YearsData) > 0) {
                array_push($last5YearsDates, $i->format("Y"));
                array_push($last5YearsData, $value);
            }
        }
        $last180DaysDates = [];
        $last180DaysPurchases = [];
        $allOrderData = Order::where('user_id', Auth::user()->id)->select("created_at", "grand_total")->get();
        for ($i = today()->subDays(179); $i <= today(); $i->addDay()) {
            $date = $i->copy()->format("Y-m-d");
            $value = $this->getOrdersSumForDate($allOrderData, $date);
            array_push($last180DaysDates, $i->format("Y-m-d"));
        ...
        //Products and Category
        $allPurchasedProducts = Xztcart::with('product')->where('user_id', Auth::user()->id)->get();
        $cartIds = [];
        foreach ($allPurchasedProducts as $item) {
            $cartIds[] = $item->product->category_id;
        }
        $cartIds = array_unique($cartIds);

        $categoryNames = [];
        $categoryPurchaes = [];
        foreach (Category::whereIn('id', $cartIds)->get() as $cat) {
            ...
            $categoryPurchaes[] = round($sum, 2);
        }

        if (Auth::user()->user_type == 'seller') {
            return view('frontend.user.seller.dashboard', compact('total_orders', 'categoryNames', 'categoryPurchaes', 'last5YearsDates', 'last5YearsData', 'last12MonthDates', 'last12MonthsPurchases', 't_orders_pending', 't_orders_cancelled', 't_orders_success', 't_products', 'last180DaysDates', 'last180DaysPurchases', 'last120DaysDates', 'last120DaysPurchases', 'last90DaysDates', 'last90DaysPurchases', 'last60DaysDates', 'last60DaysPurchases', 'last30DaysDates', 'last30DaysPurchases'));
        }
        // if(Auth::user()->user_type == 'seller'){
    ...
    public function index()
    {
        ...
        //         'name' => 'Shafiul',
        //         'user_type' => 'seller',
        //         'email' => 'dshafiul@gmail.com',
        //         'email_verified_at' => now(),
        //         'password' => Hash::make('hello1234'),
        //         'remember_token' => Str::random(10),
        //         'company_name' => 'Shafiul Shop',
        //         'phone' => '01854485855',
        //         'website' => 'shafiul.com',
        //         'state' => 1,
        //     ]);
        ...
        if (auth()->user() == null) {
            ...
            return view('backend.dashboard', compact('dealers', 'tickets', 'categoryNames', 'categoryPurchaes', 'last5YearsDates', 'last5YearsData', 'orders', 'categories', 'products', 'last12MonthDates', 'last12MonthsPurchases', 'last180DaysDates', 'last180DaysPurchases', 'last120DaysDates', 'last120DaysPurchases', 'last90DaysDates', 'last90DaysPurchases', 'last60DaysDates', 'last60DaysPurchases', 'last30DaysDates', 'last30DaysPurchases'));
        } else {
            abort(404);
        }
...
//        $categories = Category::where('level', 0)->orderBy('name', 'asc')->get();
    ...
    public function add_bulk_cart_old(Request $request, $id)
    {
        $product = Product::findOrFail($id);
        $bracket = ProductBracket::with('xztbracket')->where('product_id', '=', $id)->get();
        $springassist = ProductSpringassist::with('xztspringassist')->where('product_id', '=', $id)->get();
        ...
        return view('frontend.user.seller.bulk_cart', compact('product', 'shade_opt', 'wrap', 'bracket', 'stack', 'springassist', 'price_arr'));
    }

    public function add_bulk_cart(Request $request)
    {
        $products = Product::where("is_parts", 0)->where('state', 'Active')->where('archived', 0)->orderBy('name', 'asc')->get();

        return view('frontend.user.seller.bulk_cart', compact('products'));
    }

    // Ajax request for new row in bulk cart table.
    public function bulk_cart_row(Request $request)
    {
        ...
        if ($request->has("productId")) {
            ...
            $product = Product::where("is_parts", 0)->where('state', 'Active')->where('archived', 0)->orderBy('name', 'asc')->first();
            $id = $product->id;
        }
        $fabric = ProductFabric::with('xztfabric')->where('product_id', '=', $id)->get();
        $mount = ProductMount::with('xztmount')->where('product_id', '=', $id)->get();
        ...
        $html = view('frontend.user.seller.cart.bulk-cart-row', compact('product', 'products', 'fabric', 'price_cat', 'distinct_wid', 'distinct_len', 'fabric_all', 'shade_opt', 'fab_opt', 'coupon_arr', 'destinations', 'roomtype', 'mount', 'cassette', 'bracket', 'springassist', 'wrap', 'stack', 'mountpos', 'controltype', 'ct_manuals', 'ct_motors', 'ct_wid_motors', 'wid_motor_max', 'rowIndex', 'cust_discount', 'invoice'))->render();
    ...
    public function seller_product_list(Request $request)
    {
        ...
        return view('frontend.user.seller.products', compact('products'));
    }
...
//        if($str != null && $product->variant_product){
    ...
    public function get_cassette_color(Request $req)
    {
        ...
        foreach ($cass_color as $item) {
            echo "<option value='" . $item->color . "'>" . ucwords($item->color) . "</option>";
        }
    }
